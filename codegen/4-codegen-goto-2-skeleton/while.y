...
|
    T_ID T_COLON
    {
        ...
        goto_table[*$1] = var_data( d_loc__.first_line, goto_label, new_label() );
        $$ = new std::string("" + goto_table[*$1].label + ":\n");
        delete $1;
    }
|
    T_GOTO T_ID T_SEMICOLON
    {
        if( goto_table.count(*$2) == 0 || goto_table[*$2].decl_type != goto_label)
        {
            std::stringstream ss;
            ss << "Undeclared label: " << *$2 << std::endl;
            error( ss.str().c_str() );
        }
        $$ = new std::string("jmp near " + goto_table[*$2].label + "\n");
        delete $2;
    }
;
