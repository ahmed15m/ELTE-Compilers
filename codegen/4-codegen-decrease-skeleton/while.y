...
assignment:
    T_ID T_ASSIGN expression T_SEMICOLON
    {
        ...
        {
            std::stringstream ss;
            ss << "const variable MF cant be assigned: " << *$1 << std::endl;
            error( ss.str().c_str() );
        }
...
|
    T_DECREASING T_ID T_DO statements T_DONE
    {
...
|
    T_TIME_LIT
    {
        std::string hours = $1->substr(0,2);
        int hours_int = std::stoi(hours);
        std::string minutes = $1->substr(3,4);
        int minutes_int = std::stoi(minutes);

        std::string time = std::to_string( hours_int*256+minutes_int );
        $$ = new expression_descriptor(time_type, "mov eax, " + time + "\n");
    }
|
    T_ID
    {
...
|
    T_HOUR T_OPEN expression T_CLOSE
    {
        ...
        $$ = new expression_descriptor(integer, "" + $3->expr_code + "mov al,ah\n" + "xor ah,ah\n");
        delete $3;
    }
|
    T_MINUTE T_OPEN expression T_CLOSE
    {
        ...
        $$ = new expression_descriptor(integer, "" + $3->expr_code + "xor ah,ah\n");
        delete $3;
    }
|
	T_MAKE_TIME T_OPEN expression T_COMMA expression T_CLOSE
	{
        ...
        $$ = new expression_descriptor(time_type, "" + $3->expr_code + "mov bl,al\n" + $5->expr_code + "mov ah,bl\n" );
        delete $3;
        delete $5;
	}
|
