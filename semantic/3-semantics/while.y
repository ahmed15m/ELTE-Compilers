...
|
    T_FOR T_ID T_ASSIGN expression T_TO expression T_DO statements T_DONE
    {
		...
		{
		   std::stringstream ss;
		   ss << d_loc__.first_line << ": Variable wasn't declared before " << std::endl;
		   error( ss.str().c_str() );
		}
        if( symbol_table[*$2].decl_type != integer || *$4 != integer || *$6 != integer)
		{
		   std::stringstream ss;
		   ss << d_loc__.first_line << ": Type error. expression should be of type Integer" << std::endl;
		   error( ss.str().c_str() );
		}
        delete $2;
        delete $4;
...
|
    T_ID T_INC
    {
		...
		{
		   std::stringstream ss;
		   ss << d_loc__.first_line << "Non-integer value can not be incremented!" << std::endl;
		   error( ss.str().c_str() );
		}
        $$ = new type(integer);
        delete $1;
...
|
    T_ID T_DEC
    {
		...
		{
		   std::stringstream ss;
		   ss << d_loc__.first_line << "Non-integer value can not be decremented!" << std::endl;
		   error( ss.str().c_str() );
		}
        $$ = new type(integer);
        delete $1;
