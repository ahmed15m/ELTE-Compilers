// Generated by Bisonc++ V5.02.00 on Mon, 19 Oct 2020 18:43:12 +0200
...
PI__ const s_productionInfo[] = 
{
     ...
     {298, 1}, // 14: statement ->  label
     {298, 1}, // 15: statement ->  goto
     {305, 3}, // 16: goto (T_GOTO) ->  T_GOTO T_LABEL T_SEMICOLON
     {304, 2}, // 17: label (T_LABEL) ->  T_LABEL T_COLON
     {299, 4}, // 18: assignment (T_ID) ->  T_ID T_ASSIGN expression T_SEMICOLON
     {300, 5}, // 19: read (T_READ) ->  T_READ T_OPEN T_ID T_CLOSE T_SEMICOLON
     {301, 5}, // 20: write (T_WRITE) ->  T_WRITE T_OPEN expression T_CLOSE T_SEMICOLON
     {302, 5}, // 21: branch (T_IF) ->  T_IF expression T_THEN statements T_ENDIF
     {302, 7}, // 22: branch (T_IF) ->  T_IF expression T_THEN statements T_ELSE statements T_ENDIF
     {303, 5}, // 23: loop (T_WHILE) ->  T_WHILE expression T_DO statements T_DONE
     {306, 1}, // 24: expression (T_NUM) ->  T_NUM
     {306, 1}, // 25: expression (T_TRUE) ->  T_TRUE
     {306, 1}, // 26: expression (T_FALSE) ->  T_FALSE
     {306, 1}, // 27: expression (T_ID) ->  T_ID
     {306, 3}, // 28: expression (T_ADD) ->  expression T_ADD expression
     {306, 3}, // 29: expression (T_SUB) ->  expression T_SUB expression
     {306, 3}, // 30: expression (T_MUL) ->  expression T_MUL expression
     {306, 3}, // 31: expression (T_DIV) ->  expression T_DIV expression
     {306, 3}, // 32: expression (T_MOD) ->  expression T_MOD expression
     {306, 3}, // 33: expression (T_LESS) ->  expression T_LESS expression
     {306, 3}, // 34: expression (T_GR) ->  expression T_GR expression
     {306, 3}, // 35: expression (T_EQ) ->  expression T_EQ expression
     {306, 3}, // 36: expression (T_AND) ->  expression T_AND expression
     {306, 3}, // 37: expression (T_OR) ->  expression T_OR expression
     {306, 2}, // 38: expression (T_NOT) ->  T_NOT expression
     {306, 3}, // 39: expression (T_OPEN) ->  T_OPEN expression T_CLOSE
     {307, 1}, // 40: program_$ ->  program
};
